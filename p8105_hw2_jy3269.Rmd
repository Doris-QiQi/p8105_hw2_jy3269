...
mr <- data_2 %>% 
  ...
  mutate(origin = "Mr. Trash Wheel ",.before = "dumpster") %>% 
...
pro <- data_3 %>% 
  ...
  mutate(year=as.character(year)) %>%    # change into character to unify the format with mr.
  mutate(origin = "Professor Trash Wheel ",.before = "dumpster") %>% 

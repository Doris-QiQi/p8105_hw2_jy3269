...
## 6. Description of the data
  * the dimension of **Mr. Trash Wheel** is (`r dim(mr)`), containing `r nrow(mr)` rows and `r ncol(mr)` columns(variables)
  * variables in Mr. Trash Wheel are `r names(mr)`
  * I think the key variables in Mr. Trash Wheel are  `r names(mr)[5:6]` because they are the indicators of total weight and volume. And sports_ball may also be a variable that is  of our interest.
  
  * the dimension of **Professor Trash Wheel** is (`r dim(pro)`), containing `r nrow(pro)` rows and `r ncol(pro)` columns(variables)
  * variables in Professor Trash Wheel are `r names(pro)`
  * I think the key variables in Professor Trash Wheel are  `r names(pro)[5:6]` because they are the indicators of total weight and volume. And the professor dataset does not have the sports_ball column.
...
*  the pols data set :
  ...
  * the year range in pols is `r range(pull(pols,year))`
  * the key variable is `president`,showing which party the president is from
  
  
* the SNP data set:
  * it has `r nrow(SNP)` rows and `r ncol(SNP)` columns(variables)
  * the year range in pols is `r range(pull(SNP,year))`
  * the key variable is `close`, which means the closing values of the S&P stock index on the associated date
  
* the unemploy data set:
  * it has `r nrow(unemploy)` rows and `r ncol(unemploy)` columns(variables)
  * the year range in pols is `r range(pull(unemploy,year))`
